...
def criar_dashboard(dados):
    ...
    dados_recentes = (
        dados.sort_values("Data").groupby("Nome").last().reset_index()
    )
    st.subheader("Resumo dos Dados Mais Recentes", key="subheader_resumo_dados")
    st.dataframe(
        dados_recentes[
            ["Nome", "Data", "Peso", "Altura", "IMC", "Cintura", "Quadril"]
        ],
        key="dataframe_resumo",
    )
...
st.markdown(
    "### Acompanhe o progresso físico com base em dados de peso, medidas e parâmetros da OMS"
)

# Menu lateral
menu = st.sidebar.selectbox(
    "Escolha uma opção",
    ["Inserir Dados", "Visualizar Aluno", "Dashboard"],
    key="menu_principal",
)
...
if menu == "Inserir Dados":
    ...
    with st.sidebar.form("entrada_dados"):
        ...
        sexo = st.selectbox(
            "Sexo", ["Masculino", "Feminino"], key="select_sexo"
        )
        ...
        perc_massa_magra = st.number_input(
            "Percentual de Massa Magra (%)",
            format="%.1f",
            min_value=0.0,
            max_value=100.0,
            step=0.1,
            key="input_massa_magra",
        )
        ...
        meta_cintura = st.number_input(
            "Meta de Circunferência da Cintura (cm)",
            format="%.1f",
            min_value=0.0,
            step=0.1,
            key="input_meta_cintura",
        )
        meta_gordura = st.number_input(
            "Meta de Percentual de Gordura (%)",
            format="%.1f",
            min_value=0.0,
            max_value=100.0,
            step=0.1,
            key="input_meta_gordura",
        )

        data = st.date_input(
            "Data da medição", value=dt.date.today(), key="input_data"
        )
    ...
    if submitted:
        if nome and altura > 0 and peso > 0:
            ...
            novo_dado = pd.DataFrame(
                {
                    ...
                    "IMC": [imc],
                    "C/Q": [rcq],
                    "Percentual_Gordura": [perc_gordura],
                    "Percentual_Massa_Magra": [perc_massa_magra],
                    "Gordura_Visceral": [gordura_visceral],
                    "Meta_Peso": [meta_peso],
                    "Meta_Cintura": [meta_cintura],
            ...
            st.sidebar.success(
                f"Dados salvos com sucesso! IMC calculado: {imc}"
            )
        else:
    ...
    if not dados.empty:
        ...
        aluno_selecionado = st.selectbox(
            "Selecione um aluno", alunos, key="select_aluno_visualizar"
        )

        if aluno_selecionado:
            dados_aluno = dados[dados["Nome"] == aluno_selecionado].sort_values(
                "Data"
            )
            ...
            with col1:
                peso_atual = (
                    dados_aluno["Peso"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                meta_peso = (
                    dados_aluno["Meta_Peso"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                st.metric(
                    "Peso Atual",
                    f"{peso_atual:.1f} kg" if peso_atual is not None else "N/A",
                )
                st.metric(
                    "Meta de Peso",
                    f"{meta_peso:.1f} kg" if meta_peso is not None else "N/A",
                )
            with col2:
                imc_atual = (
                    dados_aluno["IMC"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                st.metric(
                    "IMC Atual",
                    f"{imc_atual:.1f}" if imc_atual is not None else "N/A",
                )
            with col3:
                cintura_atual = (
                    dados_aluno["Cintura"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                meta_cintura = (
                    dados_aluno["Meta_Cintura"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                st.metric(
                    "Cintura",
                    (
                        f"{cintura_atual:.1f} cm"
                        if cintura_atual is not None
                        else "N/A"
                    ),
                )
                st.metric(
                    "Meta de Cintura",
                    (
                        f"{meta_cintura:.1f} cm"
                        if meta_cintura is not None
                        else "N/A"
                    ),
                )
            with col4:
                gordura_atual = (
                    dados_aluno["Percentual_Gordura"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                meta_gordura = (
                    dados_aluno["Meta_Gordura"].iloc[-1]
                    if not dados_aluno.empty
                    else None
                )
                st.metric(
            ...
            if imc_atual is not None:
                ...
                if nivel == "success":
                    st.success(
                        f"IMC: {imc_atual:.1f} - Classificação: {classificacao}"
                    )
                elif nivel == "warning":
                    st.warning(
                        f"IMC: {imc_atual:.1f} - Classificação: {classificacao}"
            ...
            with tab1:
                ...
                if not dados_peso.empty:
                    ...
                    ax.set_title(
                        "Progresso do Peso com Faixas de Referência IMC",
                        pad=20,
                        fontsize=14,
                    )
                    ...
                    plt.xticks(
                        dados_peso["Data"],
                        dados_peso["Data"].dt.strftime("%d/%m/%Y"),
                        rotation=45,
                    )
            ...
            with tab2:
                dados_medidas = dados_aluno.dropna(
                    subset=["Cintura", "Quadril"]
                )
                if not dados_medidas.empty:
                    ...
                    for x, y in zip(
                        dados_medidas["Data"], dados_medidas["Cintura"]
                    ):
                        ax_cintura.annotate(
                            f"{y:.1f}",
                            (x, y),
                    ...
                    ax_cintura.set_title(
                        f"Medida da Cintura com Referências OMS ({sexo_atual})",
                        pad=20,
                        fontsize=14,
                    )
                    ...
                    plt.xticks(
                        dados_medidas["Data"],
                        dados_medidas["Data"].dt.strftime("%d/%m/%Y"),
                        rotation=45,
                    )
                    ax_cintura.legend(
                    ...
                    for x, y in zip(
                        dados_medidas["Data"], dados_medidas["Quadril"]
                    ):
                        ax_quadril.annotate(
                            f"{y:.1f}",
                            (x, y),
                    ...
                    ax_quadril.set_title(
                        f"Medida do Quadril ({sexo_atual})", pad=20, fontsize=14
                    )
                    ...
                    st.warning(
                        "Não há dados de medidas suficientes para gerar os gráficos"
                    )

            # Novo gráfico para Gordura Visceral
            st.subheader("Gordura Visceral")
            dados_gordura_visceral = dados_aluno.dropna(
                subset=["Gordura_Visceral"]
            )
            if not dados_gordura_visceral.empty:
                fig_gv, ax_gv = plt.subplots(figsize=(10, 6))
                ax_gv.plot(
                    dados_gordura_visceral["Data"],
                    dados_gordura_visceral["Gordura_Visceral"],
                    marker="o",
                    linewidth=2,
                    color="#2E86C1",
                )
                ...
                ax_gv.axhspan(
                    0, 9, facecolor="green", alpha=0.3, label="Normal"
                )
                ax_gv.axhspan(
                    9, 14, facecolor="yellow", alpha=0.3, label="Alto"
                )
                ax_gv.axhspan(
                    14, 30, facecolor="red", alpha=0.3, label="Muito Alto"
                )

                ax_gv.set_title(
                    "Progresso da Gordura Visceral", pad=20, fontsize=14
                )
                ...
                plt.xticks(
                    dados_gordura_visceral["Data"],
                    dados_gordura_visceral["Data"].dt.strftime("%d/%m/%Y"),
                    rotation=45,
                )
                plt.tight_layout()
                st.pyplot(fig_gv)
